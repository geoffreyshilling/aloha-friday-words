	    ...
	    if( 'afw808_aloha_friday' === $data['post_type'] ) {
	        if( empty( $data[ 'post_title'] ) ) {
				...
				*/
				...
				if (get_page_by_title( $my_title1, OBJECT, 'afw808_aloha_friday' ) != null ) {
					$my_title1 = 'Aloha Friday - Week of ' . $weekOfMonday1 . " already exists";
				} else {
					$my_title1 = 'Aloha Friday - Week of ' . $weekOfMonday1;
				}

				$timeStamp = date( 'Y-m-d H:i:s', strtotime( "$yearOfPost-$monthOfPost-$dayOfPost 08:08:00" ) ); // format needed for WordPress
